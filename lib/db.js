...
export function getPendingMediaCount() {
  return db().prepare('SELECT COUNT(*) AS count FROM media WHERE local_url IS NULL').get().count;
}

export function getNextPendingMedia(blacklist) {
  const notIn = blacklist.length > 0
    ? `AND media.id NOT IN (${blacklist.map(() => '?').join(',')})`
    : '';
  return db().prepare(`
    SELECT media.*, entries.date
    FROM media
    JOIN entries ON media.entry_id = entries.id
    WHERE media.local_url IS NULL ${notIn}
    ORDER BY entries.date DESC, media.date_index ASC
    LIMIT 1
  `).get(...blacklist) ?? null;
}

export function clearMediaLocalUrl(path) {
  return db().prepare(`UPDATE media SET local_url = NULL WHERE local_url = ?`).run(path).changes;
}

export function clearMediaLocalThumbnailUrl(path) {
  return db().prepare(`UPDATE media SET local_thumbnail_url = NULL WHERE local_thumbnail_url = ?`).run(path).changes;
}

export function updateMediaLocalPaths(id, local_url, local_thumbnail_url) {
  db().prepare('UPDATE media SET local_url = ?, local_thumbnail_url = ? WHERE id = ?')
    .run(local_url, local_thumbnail_url, id);
}

export function upsertMap({ title, date, content }) {
  db().prepare(`
    INSERT INTO maps (title, date, content)
    VALUES (?, ?, ?)
    ON CONFLICT(title, date) DO UPDATE SET content = excluded.content
